...
func main() {
	...
	http.HandleFunc("/fecharmesa", fecharmesa)
...
func fecharmesa(w http.ResponseWriter, req *http.Request){
	...
	idUs, _ := strconv.Atoi(req.FormValue("id"))
	//busca os usuario por id
	ok,mesas :=	modelos.GetMesaID(idUs)
	...
	//popula struct
	fmt.Println("ID mesa",len(mesas))
	for _, dado := range mesas {
		fmt.Println("ID mesa",dado)
		me = dado
		me.Ativa=false
	fmt.Println("ID mesa",me.ID)
	}
	for _, v := range dbSessions {
		me.Login= v.un
	}
	
	fmt.Println("Chegou aqui Fechamar mesa",idUs)
	modelos.FecharMesa(me)
